  ...
  return (
    <div className="animate-fade-in flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-surface-400 text-sm">Organize your bookstore</p>
          </div>
          <button 
            onClick={() => { resetForm(); setIsEditing(true); }}
            className="btn-brand flex items-center gap-2 lg:hidden"
          >
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-surface-700/50">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan="4" className="p-8 text-center text-surface-500">No categories found.</td>
                </tr>
              ) : (
                categories?.map((cat) => (
                  <tr key={cat._id} className="hover:bg-surface-800/30 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        <Tags className="w-4 h-4 text-brand-400" />
                        <span className="font-bold text-white">{cat.name}</span>
                      </div>
                    </td>
                    <td className="p-4 text-sm text-surface-400 hidden sm:table-cell line-clamp-1">{cat.description}</td>
                    <td className="p-4 text-center">
                      <span className="badge-brand">{cat.productCount || 0}</span>
                    </td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleEdit(cat)} className="p-1.5 text-surface-400 hover:text-blue-400 hover:bg-blue-400/10 rounded transition-colors">
      ...
      <div className={`w-full lg:w-96 shrink-0 ${isEditing ? 'block' : 'hidden lg:block'}`}>
        <div className="glass-dark rounded-2xl border border-surface-700 p-6 sticky top-24">
