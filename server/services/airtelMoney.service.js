...
class AirtelMoneyService {
  ...
  static async initiatePayment({ phoneNumber, amount, orderId, userId }) {
    ...
    if (!isValid) {
      throw new Error('Invalid Airtel Money phone number. Use 073x or 072x numbers.');
    }
    ...
    return {
      transactionId,
      status: 'pending',
      message: `Payment request sent to ${normalized}. Please check your phone and enter your Airtel Money PIN.`,
      payment,
    };
  }
  ...
  static async checkPaymentStatus(transactionId) {
    ...
    if (newStatus === 'failed') {
      ...
      payment.failureReason = 'Transaction declined or insufficient funds.';
      payment.providerResponse = {
        ...payment.providerResponse,
        status: 'FAILED',
